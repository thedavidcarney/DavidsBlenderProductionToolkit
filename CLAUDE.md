...
4. **Choose "Set as the latest release."** GitHub's release page offers these as a RADIO PAIR — "Set as a pre-release" OR "Set as the latest release" — not a checkbox, so picking one deselects the other. Pre-release = the team gets a red EXPERIMENTAL warning before updating. **Not usable until the `/releases` fix ships** (unreleased as of v1.0.18): every older client calls `/releases/latest`, which omits pre-releases entirely, so choosing it makes the release unreachable rather than merely flagged. Until then, always "latest release".
...
- **The updater lists `/releases` and picks the newest itself — NOT `/releases/latest`.** That endpoint **excludes pre-releases and drafts**, so a release marked "Set as a pre-release" is invisible to it: the check reports "you have the latest version" and the team is never offered the build they were asked to test. This bit v1.0.18 in production — it was published as a pre-release and no one could see it. Hiding a release is the wrong answer to "this is experimental"; offering it WITH the warning is. `fetch_latest_release()` skips drafts, skips non-`vX.Y.Z` tags rather than dying on them, and returns None on an empty or error response. **The fix ships in v1.0.19, so v1.0.18 itself had to be un-marked as a pre-release to reach anyone** — chicken-and-egg: a client can only use selection logic it already has.
- **The update popup shows what's new, and flags experimental releases.** The release `body` and GitHub's `prerelease` flag are stored in prefs (`latest_notes`, `latest_is_prerelease`) and rendered by `draw_release_info()`, shared between the popup and the sidebar panel so the two cannot drift. **The experimental warning is driven by the "Set as a pre-release" radio option in GitHub's release UI** (it pairs with "Set as the latest release" — selecting one clears the other) — choose it and the team sees a red EXPERIMENTAL box telling them to finish the job first; choose "latest release" for a release that's been proven. This is why the one-line release note matters: it is what the team actually reads before deciding to update. Both fields are read with `.get()` so a release with no body, or an API response without the field, can never break the check itself. Long bodies are clamped (`MAX_NOTE_LINES`, `NOTE_LINE_WIDTH`) so a future paragraph cannot stretch the popup or push the buttons off screen.
...
- **`run_camera_overlay_test.sh`** — the only suite that tests the GPU for real. On 5.2 it calls `gpu.init()` to get a headless GL context, compiles the overlay shader, draws into a `GPUOffScreen` and **reads the pixels back**. Add `--gpu-backend vulkan` to the Blender invocation to run the identical suite on Vulkan — **both backends verified green**, which is what makes the "no backend branching" rule safe to rely on (David's machine is Vulkan/NVIDIA; `BOOL` push constants were the specific worry and they are fine): opacity 0/0.5/1.0, mask above and below threshold, invert, per-channel selection, and tint. Also checks the fit/scale/offset/rotation/flip maths against a known frame, that 20 enable/disable cycles leak no handler, and (by AST walk) that nothing reads `.pixels`. On 4.5/5.0/5.1 there is no `gpu.init`, so the GPU phases are skipped with a printed NOTE and the rest still runs — a skip is reported, never silently passed.
