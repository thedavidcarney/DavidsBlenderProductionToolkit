...
4. **Decide whether to tick "Set as a pre-release."** Ticked = the team gets a red EXPERIMENTAL warning before updating. Tick it for anything not yet proven in a real job, especially mid-season. Untick once it has been run in anger. **Only safe from v1.0.19 on** — every client older than that calls `/releases/latest`, which hides pre-releases entirely, so ticking it there makes the release unreachable rather than merely flagged.
